import os
import sys
from PyQt6.QtWidgets import QMainWindow, QWidget, QDial, QGridLayout, QLabel, QPushButton, QCheckBox
...
class DispApp(QMainWindow):
    def __init__(self, ctx):
        ...
        self.widgets = [
            ...
            [QDial(),120,120, 1,0, 1,1],
            [QLabel(),120,10, 2,0, 1,1],
            [QDial(),120,120, 3,0, 1,1],
            [QLabel(),120,10, 4,0, 1,1],
            [QDial(),120,120, 5,0, 1,1],
    ...
    def redraw(self, fn=None):
        image = np.zeros((800*800*4), dtype=np.int32)
        ...
        self.knl(self.queue, self.x.shape, None, self.d_x, self.d_y, self.d_z, d_image, np.int32(imw), np.int32(imh), np.float32(al),np.float32(be),np.float32(mu), np.int32(self.widgets[7][0].checkState().value))

        cl.enqueue_copy(self.queue, image, d_image)
        im2 = np.array(image, dtype=np.float64)
        if self.widgets[6][0].checkState().value:
            im2 = np.log(1.0 + 10.0 * im2 / max(im2))
        im3 = np.round(65535.0-(65535.0 * im2 / max(im2)))
        im4 = np.array(im3, dtype=np.uint16)
        im4.reshape((800,800,4))

        qim = QImage(im4.data, imw, imh, QImage.Format.Format_RGBA64)
        if fn != None:
            qim.save(fn,'PNG')
